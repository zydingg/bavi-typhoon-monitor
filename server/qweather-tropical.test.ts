...
test('maps active NP list, track, and forecast', async () => {
  ...
  const storms = await createQWeatherLoader({ apiKey: 'test-key', fetcher, now: () => new Date('2026-07-11') })();
...
test('rejects a QWeather payload code other than 200', async () => {
  const fetcher = vi.fn()
    ...
    .mockResolvedValueOnce(json({ code: '204', storm: [] }));
  const load = createQWeatherLoader({ apiKey: 'test-key', fetcher, now: () => new Date('2026-07-11') });
...
test('rejects a missing API key without making an upstream request', async () => {
  const fetcher = vi.fn();
  const load = createQWeatherLoader({ apiKey: '', fetcher });
...
test('aborts a hung QWeather request after the configured timeout', async () => {
  ...
  const load = createQWeatherLoader({ apiKey: 'test-key', fetcher, timeoutMs: 10 });
